...
//----
   CalculateOsMA(limit);
   CalculateDivergence(limit,Bars);
//----
   return(0);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void CalculateDivergence(int countedBars,int maxind)
  {
   ...
   for(int i=countedBars; i>=0; i--)
     {
      ...
      //----   
      int firstPeakOrTroughShift=GetFirstPeakOrTrough(i,maxind);
      if(firstPeakOrTroughShift==-1) return;

      double firstPeakOrTroughOsMA=OsMA[firstPeakOrTroughShift];
      if(firstPeakOrTroughOsMA>0)
        {
         int peak_0  =GetIndicatorLastPeak(i,maxind);
         int trough_0=GetIndicatorLastTrough(peak_0,maxind);
         int peak_1  =GetIndicatorLastPeak(trough_0,maxind);
         int trough_1=GetIndicatorLastTrough(peak_1,maxind);
        }
      else
        {
         trough_0=GetIndicatorLastTrough(i,maxind);
         peak_0  =GetIndicatorLastPeak(trough_0,maxind);
         trough_1=GetIndicatorLastTrough(peak_0,maxind);
         peak_1  =GetIndicatorLastPeak(trough_1,maxind);
        }
      //----  
...
void CalculateOsMA(int countedBars)
  {
   ...
   for(i=countedBars; i>=0; i--)
     {
      Signal[i]=iMAOnArray(MACD,0,signal,0,MODE_SMA,i);
...
//|                                                                  |
//+------------------------------------------------------------------+
int GetPositiveRegionStart(int index,int maxind)
  {
   int regionStart;
   for(int i=index+1; i<maxind-2; i++)
     {
...
//|                                                                  |
//+------------------------------------------------------------------+
int GetNegativeRegionStart(int index,int maxind)
  {
   for(int i=index+1; i<maxind-2; i++)
     {
      if(OsMA[i]<=OsMA[i-1] && OsMA[i]<=OsMA[i+1] &&
...
//|                                                                  |
//+------------------------------------------------------------------+
int GetFirstPeakOrTrough(int index,int maxind)
  {
   for(int i=index+1; i<maxind-2; i++)
     {
      if((OsMA[i]>=OsMA[i-1] && OsMA[i]>=OsMA[i+1] && 
...
//|                                                                  |
//+------------------------------------------------------------------+
int GetIndicatorLastPeak(int index,int maxind)
  {
   int regionStart=GetPositiveRegionStart(index,maxind);
   if(regionStart==-1)
      return(-1);
//----   
   int peakShift=0;
   double peakValue=0;
//----
   for(int i=regionStart; i<maxind-2; i++)
     {
...
//|                                                                  |
//+------------------------------------------------------------------+
int GetIndicatorLastTrough(int index,int maxind)
  {
   int regionStart=GetNegativeRegionStart(index,maxind);
   if(regionStart==-1)
      return(-1);
//----         
   int troughShift=0;
