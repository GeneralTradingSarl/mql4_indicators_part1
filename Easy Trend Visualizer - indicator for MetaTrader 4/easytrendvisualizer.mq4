...
//---
   string DateTime=TimeYear(Time[0])+"-"+FormatDateTimeDigit(TimeMonth(Time[0]))+"-"+FormatDateTimeDigit(TimeDay(Time[0]))+" "+FormatDateTimeDigit(TimeHour(Time[0]))+":"+FormatDateTimeDigit(TimeMinute(Time[0]));
   string PerStr=PeriodToString(Period());
   if(UseAlertHorizontalLine)
     {
      if((Ex[1]!=EMPTY_VALUE) && (Ex[1]!=was_alert_hl) && (Ex[1]!=Ex[2]))
        {
         Alert("ETV-HL "+DateTime+" "+Symbol()+" @ "+PerStr);
         was_alert_hl=Ex[1];
        }
     }
   if(UseAlertUpDownArrows)
     {
      if((Up[0]!=EMPTY_VALUE) && (Up[0]!=was_alert_au))
        {
         Alert("ETV-AU "+DateTime+" "+Symbol()+" @ "+PerStr);
         was_alert_au=Up[0];
        }
      if((Dn[0]!=EMPTY_VALUE) && (Dn[0]!=was_alert_ad))
        {
         Alert("ETV-AD "+DateTime+" "+Symbol()+" @ "+PerStr);
         was_alert_ad=Dn[0];
        }
     }
//---
